...
class NetWork(nn.Module):
    ...
    def __init_weight(self):
        ...
        # nn.init.trunc_normal_(self.fc3.weight, std=0.1)
        # nn.init.trunc_normal_(self.fc4.weight, std=0.1)
        nn.init.trunc_normal_(self.out.weight, std=0.1)

        nn.init.trunc_normal_(self.fc1.bias, 0.1)
        nn.init.trunc_normal_(self.fc2.bias, 0.1)
        # nn.init.trunc_normal_(self.fc3.bias, 0.1)
        # nn.init.trunc_normal_(self.fc4.bias, 0.1)
...
class DQNAgent():
    ...
    def train(self):
        if os.path.exists(self.config.save_dir+"/"+self.config.model_path):
            self.network.load_state_dict(self.network.state_dict(),torch.load(self.config.save_dir+"/"+self.config.model_path))
            self.eval_network.load_state_dict(self.eval_network.state_dict(),torch.load(self.config.save_dir+"/"+self.config.model_path))
            print("加载成功")
